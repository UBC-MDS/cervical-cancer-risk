...
*Disclaimer: The current project is only for educational purposes. It shall not be used for actual diagnoses.*
...
2.  Create the environment:
    ...
    <https://github.com/UBC-MDS/cervical-cancer-group7/blob/main/environment.yaml>
...
4.  Use the following MAKE commands:
    ...
    To run an individual script (see manual scripts section below)

    `Make <script>`

NOTE: if the render on the analysis document fails, ensure that `tidyverse` and `rmarkdown` are installed on your computer. See step 7 of "Manually Run Scripts" below.
...
To manually run the data analysis on your local device, follow these steps. These scripts can be run in their current quoted form and will automatically generate the files on your device without manipulating the arguments.
...
`Rscript -e "install.packages('rmarkdown'), repos='https://cran.rstudio.com/')"` (ensure Rmarkdown is installed)
